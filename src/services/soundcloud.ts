  ...
  async function findClientID() {
    try {
      const sc = await fetch('https://soundcloud.com/')
        .then((r) => r.text())
        .catch((e) => {
          const isBlocked = (e as Error).cause?.toString().includes('ECONNRESET');
          if (isBlocked)
            core.log.warn(
              'Soundcloud may be blocked in your region. Use proxy or VPN to bypass restrictions.'
            );
          return '';
        });

      const scVersion = String(
        sc.match(/<script>window\.__sc_version="[0-9]{10}"<\/script>/)?.[0].match(/[0-9]{10}/)
      );
  ...
  return {
    ...
    fetchMediaInfo: async (url, args) => {
      ...
      if (new URL(url).hostname === 'on.soundcloud.com' && shortLink) {
        link = await fetch(`https://on.soundcloud.com/${shortLink}/`, { redirect: 'manual' })
          .then((r) => {
            if (
              r.status === 302 &&
              r.headers.get('location')?.startsWith('https://soundcloud.com/')
            ) {
      ...
      const json = await fetch(
        `https://api-v2.soundcloud.com/resolve?url=${link}&client_id=${clientId}`
      )
      ...
      let bestAudio = 'opus';
      let selectedStream = json.media.transcodings.find((v: any) => v.preset === 'opus_0_0');
      const mp3Media = json.media.transcodings.find((v: any) => v.preset === 'mp3_0_0');
